...
async function getJson(){
  let rawData = await fetch('/books.json')
  let data = await rawData.json()
  return data
}

async function getTheElement() {
  await fetch('/html/pages/boot-strap.html')
    .then(response => response.text())
    .then(data => {
      const parser = new DOMParser();
      const bootstrapDoc = parser.parseFromString(data, 'text/html');
      const myComponent = bootstrapDoc.querySelector('main');
      ...
      getJson().then((data) => {
        ...
        myComponent.innerHTML = cardHtml;
      }).catch((error) => {
        console.error(error);
      });
...
/* html += `<div class="card" style="width: 18rem;">`
    html += `<img src="img\book${counter}.jpg" class="card-img-top">`;
    html += `<div class="card-body">`
    html += `<h5 class="card-title">${books.title}</h5>`;
